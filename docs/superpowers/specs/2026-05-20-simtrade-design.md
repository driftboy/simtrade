...
| 触发条件 | 动作 | 说明 |
|---------|------|------|
| 双方确认签约 | `Transaction: 待签约 → 已签约` | 询盘/发盘完成 |
| 合同生效 | `Transaction: 已签约 → 履约中` | 进入履约阶段 |
| 信用证已通知 | `Transaction: 履约中` (确认) | 支付保障生效 |
| 所有必需单证归档 | `Transaction: 履约中 → 已完成` | 履约完毕 |
| 任一方取消 | `Transaction: * → 已取消` | 任何状态都可取消 |
| 超时未操作 | `Transaction: 询盘中 → 已取消` | 自动清理 |

#### Contract (合同) 联动

| 触发条件 | 动作 | 说明 |
|---------|------|------|
| 出口商创建 | `Contract: 草稿` | 关联交易 |
| 双方签字 | `Contract: 待签字 → 双方已签字` | 需双方确认 |
| 双方签字完成 | `Contract: 双方已签字 → 已生效` | 自动生效 |
| 合同生效 | 自动创建 `LetterOfCredit: 草稿` | 如需信用证 |
| 履约完成 | `Contract: 已生效 → 履行完毕` | 单证齐全后 |

#### LetterOfCredit (信用证) 联动

| 触发条件 | 动作 | 说明 |
|---------|------|------|
| 合同生效 | 自动创建 `L/C: 草稿` | 由系统创建 |
| 进口商申请开证 | `L/C: 草稿 → 待开证` | 提交银行 |
| 银行开证 | `L/C: 待开证 → 已开证` | 银行处理 |
| 出口商确认 | `L/C: 已开证 → 已通知` | 通知出口商 |
| 议付完成 | `L/C: 已通知 → 待议付 → 已议付` | 银行审核 |
| 付款完成 | `L/C: 已议付 → 已付款` | 结算完成 |
| L/C 已通知 | `Transaction: 履约中` | 确保状态同步 |

#### Document (单证) 联动

| 触发条件 | 动作 | 说明 |
|---------|------|------|
| 用户创建 | `Document: 草稿` | 关联交易 |
| 提交审核 | `Document: 草稿 → 待审核` | 学生提交 |
| 审核通过 | `Document: 待审核 → 已审核` | 教师/系统审核 |
| 审核不通过 | `Document: 待审核 → 草稿` | 返回修改 |
| 提交对方 | `Document: 已审核 → 已提交` | 发送给交易对手 |
| 对方接收 | `Document: 已提交 → 已接收` | 对方确认 |
| 归档 | `Document: 已接收 → 已归档` | 流程结束 |
| 所有必需单证归档 | 触发 `Transaction: 履约中 → 已完成` | 检查完整性 |

#### ProductionOrder (生产订单) 联动

| 触发条件 | 动作 | 说明 |
|---------|------|------|
| 出口商下单 | `Order: 待确认` | 工厂接收 |
| 工厂确认 | `Order: 待确认 → 已确认` | 接受订单 |
| 开始生产 | `Order: 已确认 → 生产中` | 工厂操作 |
| 生产完成 | `Order: 生产中 → 已完成` | 可发货 |
| 发货 | `Order: 已完成 → 已发货` | 通知出口商 |
| 发货完成 | 自动创建 `Document: 装箱单(草稿)` | 触发单证制作 |

#### Payment (付款) 联动

| 触发条件 | 动作 | 说明 |
|---------|------|------|
| 发起付款 | `Payment: 待支付` | 创建支付记录 |
| 银行处理 | `Payment: 待支付 → 处理中` | 银行处理 |
| 付款成功 | `Payment: 处理中 → 已支付` | 付款方 |
| 收款确认 | `Payment: 已支付 → 已收款` | 收款方确认 |
| 付款取消 | `Payment: * → 已取消` | 任一方可取消 |

#### Course (课程) 联动

| 触发条件 | 动作 | 说明 |
|---------|------|------|
| 教师创建 | `Course: 筹备中` | 初始状态 |
| 开放报名 | `Course: 筹备中 → 报名中` | 学生可加入 |
| 开始上课 | `Course: 报名中 → 进行中` | 正式开始 |
| 结束课程 | `Course: 进行中 → 已结束` | 教师操作 |
| 课程结束 | 自动触发 `Score: 计算` | 计算所有学生成绩 |
| 成绩发布 | `Course: 已结束 → 已归档` | 完成全部流程 |

#### Score (成绩) 联动

| 触发条件 | 动作 | 说明 |
|---------|------|------|
| 课程结束 | `Score: 计算中 → 待审核` | 系统自动计算 |
| 教师审核 | `Score: 待审核 → 已发布` | 教师确认 |
| 学生确认 | `Score: 已发布 → 已确认` | 完成流程 |
| 学生申诉 | `Score: 已发布 → 待审核` | 重新审核 |
...
| 数据类型 | 说明 | 数据量 | 来源 |
|---------|------|--------|------|
| 国家信息 | 各国基本信息 | 200+ 条 | ISO 3166 标准 |
| 港口信息 | 主要贸易港口 | 500+ 条 | UN/LOCODE 代码 |
| 汇率 | 主要货币汇率 | 20+ 条 | 实时 API 或定期更新 |
| 运费率 | 各航线运费标准 | 100+ 条 | 初始配置+后台维护 |
| 保险费率 | 各险种费率 | 20+ 条 | 初始配置+后台维护 |
| 商品库 | 示例商品 | 200+ 条 | 初始配置+教师扩展 |
...
#### 方式一：Django Management Command

```bash
python manage.py init_data
```

自动加载 `fixtures/` 目录下的 JSON/YAML 数据文件。

#### 方式二：SQL 脚本

提供 `init_data.sql` 脚本，直接执行导入基础数据。

#### 方式三：管理后台

管理员通过 Django Admin 手动录入或导入。
...
```
fixtures/
├── initial_data.json          # 综合初始数据
├── countries.json             # 国家数据
├── ports.json                 # 港口数据
├── currencies.json            # 货币数据
├── roles.json                 # 角色权限
├── documents.json             # 单证类型和模板
└── products.json              # 示例商品
```
...
| 1.2 | 2026-05-21 | 修复评审问题：数据表计数、错误码体系、数据初始化方案、完整状态联动规则 |
